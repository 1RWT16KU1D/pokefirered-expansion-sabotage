...
{
    // Passive Traps
    TRAP_TRAPPERS_TERRITORY  = 1,  // All Pokémon cannot switch out
    TRAP_GRAVEYARD           = 2,  // All Pokémon gain Ghost-type (can have up to 3 types)
    TRAP_WRONG_INTUITION     = 3,  // Pokémon will use a different move than intended
    TRAP_ITS_RAINING_HAZARDS = 4,  // Random entry hazards are set at the start of each turn
    TRAP_BALLROOM_BONANZA    = 5,  // All Pokémon are infatuated, provided both genders exist on field
    TRAP_ABILITY_NULL        = 6,  // All abilities are nullified
    TRAP_CHILLY_WINDS        = 7,  // All moves deal additional Ice-type damage
    TRAP_HEALING_GROUNDS     = 8,  // All recovery moves heal double the amount
    TRAP_BIZZARE_TEMPLE      = 9,  // 50% chance to switch moves with opponent
    TRAP_HOT_LAVA            = 10, // All Pokémon are burned, except Fire-types
    TRAP_STAT_ROULETTE       = 11, // All Pokémon gain a random 2-stage stat boost
    TRAP_COFFEE_FIELDS       = 12, // Pokémon can't sleep; Speed either halved or doubled
    TRAP_DISTORTED_FURY      = 13, // Physical moves become Special and vice-versa

    TRAP_PASSIVE_COUNT,

    // Active Traps
    TRAP_NO_INTERFERENCE     = 14, // Using an item cancels the effect and ends the turn
    TRAP_SHATTERED_DREAMS    = 15, // Mega/Z/Dyna/Tera fail and turn ends
    TRAP_JACKPOT             = 16, // 25% chance to guarantee a critical hit
    TRAP_BANANA_PEEL         = 17, // Using a Grass-type move forces the user to switch out
    TRAP_NO_HOLDS_BARRED     = 18, // Fighting-type moves deal double damage, accuracy halved
    TRAP_SEALED_DEMON        = 19, // For that turn, all moves have double power
    TRAP_BLOOD_PACT          = 20, // KOing an opponent costs half the user's HP
    TRAP_SIN_OF_PRIDE        = 21, // All Dragon-type moves become Dark-type
    TRAP_DETONATION_DELUSION = 22, // 25% chance for any move to become Explosion
    TRAP_NOTHING_SPECIAL     = 23, // Normal-type moves become a random type
    TRAP_TOTAL_COUNT
};
