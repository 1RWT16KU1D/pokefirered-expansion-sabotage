...
#define B_FLAG_DYNAMAX_BATTLE       0x900     // If this flag is set, the ability to Dynamax in battle is enabled for all trainers.
#define B_FLAG_TERA_ORB_CHARGED     0x902     // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
#define B_FLAG_TERA_ORB_NO_COST     0x903    // If this flag is set, the Tera Orb does not use up its charge upon Terastallization. In S/V, this occurs after an event with Terapagos.
...
#define B_FLAG_SABOTAGE_BATTLE      0x901 // If this flag is set, the battle is a sabotage battle where the opponent's Pokémon have held items that negatively affect the player's Pokémon when they are switched in.
